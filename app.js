...
function parseLooseResultLine(line){
 ...
 const times=[...clean.matchAll(/(?<!\d)(\d{1,2})\s*[:：]\s*([0-5]\d)(?!\d)/g)];
 ...
 let name=beforeFirstTime
   .replace(/\b(?:S|A|B|C|D)?\s*\d+조\b/gi," ")
   .replace(/\b(?:여성조|일반조|회원조|게스트|none조)\b/gi," ")
   .replace(/[,:/|-]+$/g,"")
   .trim();
 if(!name){
...
$("#pasteArea").addEventListener("input",()=>{
 ...
 pasteParseTimer=setTimeout(()=>{previewRows=parseText($("#pasteArea").value);showPreview()},120);
});
